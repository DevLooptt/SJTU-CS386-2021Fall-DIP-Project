...
parser.add_argument("--dataset-dir", default='.', type=str, help="Dataset directory") # 测试集图片存放目录地址
parser.add_argument("--dataset-list", default=None, type=str, help="Dataset list file") # 测试集图片名txt地址
...
def main():
    ...
    # 仅需要输出测试图片的深度图时，使用如下代码
    from kitti_eval.mytest_depth_utils import test_framework_KITTI as test_framework


    disp_net = getattr(models, args.dispnet)().cuda()
    weights = torch.load(args.pretrained_dispnet)
    disp_net.load_state_dict(weights['state_dict'])
    disp_net.eval() # 将模型设置成evaluation模式
    ...
    # test_files 为测试集的说明，即列出了测试集的文件名
    framework = test_framework(dataset_dir, test_files, seq_length, args.min_depth, args.max_depth) #TODO 修改test_framework以适配我们的测试集

    print('{} files to test'.format(len(test_files)))
    # errors = np.zeros((2, 7, len(test_files)), np.float32)
    # if args.output_dir is not None:
    #     output_dir = Path(args.output_dir)
    #     viz_dir = output_dir/'viz'  #TODO what is viz?
    ...
    for j, sample in enumerate(tqdm(framework)):
        ...
        pred_disp = disp_net(tgt_img_var) #用disp_net获取tgt_img_var的预测视差pred_disp
