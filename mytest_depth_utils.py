...
class test_framework_KITTI(object):
    def __init__(self, root, test_files, seq_length=3, min_depth=1e-3, max_depth=100, step=1):
        ...
        self.img_files = my_read_scene_data(self.root, test_files, seq_length, step) #TODO
    ...
    def __getitem__(self, i):
        tgt = imageio.imread(self.img_files[i][0]).astype(np.float32) # 这里读取图片时，只用到了img_files[i][0]，即tgt_img_path，与ref无关
